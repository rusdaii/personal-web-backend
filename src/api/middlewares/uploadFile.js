...
const uploadImage = multer({
  ...
  fileFilter: fileFilter(['image/png', 'image/jpg', 'image/jpeg', 'image/gif']),
});
...
const uploadIcon = multer({
  ...
  fileFilter: fileFilter(['image/svg+xml']),
});
...
const upload =
  (type, multerConfig) => (engine, options) => (req, res, next) => {
    let uploadEngine;
    switch (type) {
      case 'image':
      ...
      if (err) {
        if (err instanceof multer.MulterError) {
          if (err.code === 'LIMIT_UNEXPECTED_FILE') {
            next(
              new InvariantError('File you upload exceeds the maximum limit')
            );
          }
        }
